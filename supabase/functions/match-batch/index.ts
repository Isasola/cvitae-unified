...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: profile, error: profileError } = await supabase.from('user_master_profiles').select('professional_title, profile_data, is_subscribed, match_alerts_enabled').eq('user_id', user.id).maybeSingle();
