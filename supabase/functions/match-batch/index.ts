...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    // Fresh embeddings are generated by the embed-profile Edge Function on profile save
    let embedding: number[] | null = Array.isArray(profile.embedding) ? profile.embedding : null;
    if (!embedding) {
      // Fallback: attempt to generate and cache (first-time users without cached embedding)
