...
const analyticsPolicy = read('netlify/functions/lib/admin-analytics-policy.ts')
...
assert(analytics.includes('analyticsAiRequested(body)'), 'admin-analytics must centralize its explicit AI gate')
assert(analyticsPolicy.includes('value.includeAi === true'), 'Gemini requires an explicit request')
assert(growth.includes('gemini_configured'), 'Analytics must show backend Gemini configuration, never a browser key')
assert(!growth.includes('VITE_GEMINI'), 'The browser must not inspect or receive a Gemini API key')
