  ...
  return (
    <div className="flex flex-col items-center justify-center py-8">
      <div className="relative mb-6" style={{ width: blockSize * 4, height: blockSize * 4 }}>
        ...
        <div className="absolute inset-0 border border-gold/20 rounded-sm grid grid-cols-4 gap-px opacity-20">
          {[...Array(16)].map((_, i) => <div key={i} className="border border-gold/10" />)}
        </div>
      </div>
      {text && <p className="text-gold font-medium text-sm">{text}</p>}
    </div>
  )
}
