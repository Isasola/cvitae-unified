      ...
      <AnimatePresence mode="wait">
        {!hasAnalyzed ? (
          <motion.div key="upload" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
            <GlassCard className="text-center p-8 md:p-12">
              <div onClick={() => !isAnalyzing && fileInputRef.current?.click()}
                className={`border-2 border-dashed rounded-2xl p-12 transition-all cursor-pointer ${isAnalyzing ? 'opacity-50 pointer-events-none' : 'border-gold/20 hover:border-gold/50 hover:bg-gold/5'}`}>
                ...
                <p className="text-muted text-sm">PDF o Word · Máx 15 MB</p>
              ...
              <GlassCard>
                ...
                <ul className="space-y-2">
                  {results.strengths?.slice(0, 3).map((s, i) => (
                    <li key={i} className="text-sm text-muted flex items-start gap-2"><span className="text-green-400 font-bold">•</span> {s}</li>
                  ))}
                </ul>
              </GlassCard>
              <GlassCard>
                ...
                <ul className="space-y-2">
                  {results.criticalImprovements?.slice(0, 3).map((s, i) => (
                    <li key={i} className="text-sm text-muted flex items-start gap-2"><span className="text-yellow-400 font-bold">•</span> {s}</li>
                  ))}
                </ul>
              </GlassCard>
            </div>
            <div className="bg-gold/10 border border-gold/30 rounded-2xl p-6 text-center">
              ...
              <div className="flex items-center justify-center gap-2 max-w-md mx-auto">
                <input
                  ...
                  className="flex-1 px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-[#888888] focus:outline-none focus:border-gold/50"
                />
          ...
          <GlassCard className="text-center py-8">
            ...
            <p className="text-muted mb-4">{results?.error || 'Ocurrió un error inesperado.'}</p>
